...
import { FaHeart, FaChevronDown, FaUser, FaSignOutAlt } from 'react-icons/fa';
import { useAuth } from '../context/AuthContext'; 

const Navbar = () => {
  const [isDropdownOpen, setDropdownOpen] = useState(false); // Added state back for dropdown
      ...
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
        ...
        <div className="nav-center">
          <div className="nav-links">
            ...
            {/* CONDITIONAL RENDERING: Only show Check PCOS if logged in */}
            {isLoggedIn && (
              <div 
        ...
        <div className="nav-right">
          {isLoggedIn ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
              <span style={{ fontWeight: '600', color: '#D6689C' }}>Hi, User</span>
              <button onClick={handleLogout} className="btn-outline" style={{ padding: '8px 20px', fontSize: '0.9rem' }}>
                <FaSignOutAlt /> Logout
              </button>
            </div>
          ) : (
